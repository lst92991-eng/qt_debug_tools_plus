...
完整版本是行为参考，不是复制来源。遇到不理解的代码时，可在 Qt Creator 的 Git 工具或终端中查看单个文件：

```powershell
git show dev:src/sdk/DataFrame.h
git diff teaching/from-zero..dev -- src/sdk/DataFrame.h
```

先写出自己的理解和接口，再对照 `dev` 查漏。协议 ID、字节长度、端序、CRC、握手、超时和重试绝不能凭印象填写。
