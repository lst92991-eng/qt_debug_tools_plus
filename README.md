...
当前状态：安装步骤已经写全，最小工程已在现有开发机验证；仍需在一台未安装 Visual Studio、Qt 和 VS Code Qt 扩展的干净 Windows 机器或虚拟机上完整走一遍。完成该验证前，本课不标记为最终版。Git 不属于第一章必备环境。

## 第一步运行目标

学生在 Qt Creator 中选择 `Desktop Qt 6.11.2 MSVC2022 64bit` Kit，通过向导创建 `D:\QtProjects\MCUDebugTool`，完成 Debug 构建并看到空白 `MainWindow`，即表示环境正常。
